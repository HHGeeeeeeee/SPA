...
export function CustomerPaymentCard({
  ...
  const paymayaId = paymentMethods.find((p) => p.code?.toLowerCase() === 'paymaya')?.id ?? null;
  const svcId = paymentMethods.find((p) => p.code?.toLowerCase() === 'stored_value_card')?.id ?? null;
