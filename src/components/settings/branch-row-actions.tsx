...
interface Props {
  branch: { id: string; code: string; name: string; business_unit_ids: string[]; reservation_enabled: boolean; open_time: string | null; close_time: string | null; therapist_share_group: string | null; commission_policy_id: string | null; commission_rate_overrides: { commission_class_id: string; rate: number }[]; active: boolean };
...
export function BranchRowActions({ branch, businessUnits, commissionPolicies, commissionClasses, shareGroupSuggestions }: Props) {
  ...
  return (
    <>
      ...
      <BranchFormDialog
        mode="edit"
        branch={{ id: branch.id, code: branch.code, name: branch.name, business_unit_ids: branch.business_unit_ids, reservation_enabled: branch.reservation_enabled, open_time: branch.open_time, close_time: branch.close_time, therapist_share_group: branch.therapist_share_group, commission_policy_id: branch.commission_policy_id, commission_rate_overrides: branch.commission_rate_overrides }}
