#!/usr/bin/env node
// One-off: insert (or refresh) the "Order & service status" help article.
// Usage: node scripts/seed-help-status-article.mjs
...
const SLUG = 'order-and-service-status';
const content = readFileSync(resolve(__dirname, 'help-status-article.md'), 'utf8');

const row = {
  slug: SLUG,
  title: 'Order & service status — what each state means for money',
  category: 'daily_ops',
